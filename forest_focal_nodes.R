...
for_cov_p <- shapefile("C:/Users/saraw/Documents/SEARRP/processed_spat_data/forest_clumps/for_cov_p.shp")  
for_cov_sf <- st_as_sf(for_cov_p)
sabah_sf <- st_as_sf(border_sabah_d)

#  Plot
plot_for <- ggplot() +
	geom_sf(data = sabah_sf, colour = "grey90", fill = "transparent") +
	geom_sf(data = for_cov_sf, aes(fill = factor(DN)), colour = "transparent") +
	scale_fill_manual(values = c("grey30", "darkgreen", "yellowgreen",  "yellow1", "wheat3", "deepskyblue3"),
...
for_cov_pu <- gUnionCascaded(for_cov_p)

tst <- gTouches(

lps <- coordinates(for_cov_p)
ID <- cut(lps[,1], quantile(lps[,1]), include.lowest=TRUE)
for_cov_pu <- unionSpatialPolygons(for_cov_p, ID)
